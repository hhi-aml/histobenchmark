...
def TransformerPatchTokenizer(dim_model, patch_size=16, input_channels=3, dim_data=2, return_batch_first=False):
    '''standard ViT Tokenizer'''
    return TransformerTokenizer(
        strides=[patch_size], kss=[patch_size], features=[dim_model], paddings=[0], max_pool=False, activation=None,
        input_channels=input_channels, dim_data=dim_data, return_batch_first=return_batch_first
    )

def TransformerSingleConvTokenizer(dim_model, input_channels=3, kernel_size=7, stride=2, padding=3, pooling_kernel_size=3, pooling_stride=2, pooling_padding=1, max_pool=True, activation="relu",dim_data=2,return_batch_first=False):
    '''CompactTransformer Tokenizer'''
    return TransformerTokenizer(
        strides=[stride], kss=[kernel_size], features=[dim_model], paddings=[padding], max_pool=max_pool,
        activation=activation, input_channels=input_channels, dim_data=dim_data, return_batch_first=return_batch_first
    )
...
class TransformerTokenizer(nn.Module):
    ...
    def __init__(self,
                 ...
                 input_channels=3,
                 activation="gelu",dim_data=2,return_batch_first=False):
        ...
        for inf,outf,ks,stride,padding in zip(in_filters,out_filters,kss,strides,paddings):
            layers.append(conv_fn(inf, outf, kernel_size=ks, stride=stride, padding=padding))
        ...
        self.kwargs = {"dim_data":dim_data, "kss":kss, "strides": strides, "paddings": paddings, "max_pool": max_pool, "pooling_ks": pooling_ks, "pooling_stride": pooling_stride, "pooling_padding": pooling_padding} 
...
class TransformerPooling(nn.Module):
    def __init__(self,dim_model,pooling_type="cls",cls_token_first=False,batch_first=False,n_heads_seq_pool=1,layer_norm=True):
        super().__init__()
...
class TransformerHead(nn.Module):
    def __init__(self,dim_model,num_classes,pooling_type="cls",cls_token_first=False,batch_first=False,n_heads_seq_pool=1):
        super().__init__()
...
class TransformerModule(nn.Module):
    ...
    def __init__(self, dim_model=768, mlp_ratio = 4.0, dropout=0.1, dropout_attention=0.1, stochastic_depth_rate=0.1,
                 num_layers=12, num_heads=12, masked=False, max_length=1024, batch_first=False, input_size=None, 
                 output_size=None, activation='gelu', pos_enc="sine", cls_token=False, native=False):
        super().__init__()
        ...
        self.transformer_encoder = nn.ModuleList([
            nn.TransformerEncoderLayer(d_model=dim_model, nhead=num_heads, dim_feedforward=dim_feedforward,
                                       dropout=dropout, activation=activation) if native 
            else TransformerEncoderLayer(d_model=dim_model, nhead=num_heads, dim_feedforward=dim_feedforward, 
                                         dropout=dropout, attention_dropout=dropout_attention, drop_path_rate=dpr[i], 
        ...
        self.fc_in = nn.Linear(input_size,dim_model) if input_size is not None else None
        self.fc_out = nn.Linear(dim_model,output_size) if output_size is not None else None
    ...
    def attention_map(self, x, layer=-1, shape_P=None):
        if(self.batch_first): # (B, Sx, E) otherwise (Sx, B, E)
            x = x.permute(1, 0, 2)  # (Sx, B, E)
        ...
        for i, blk in enumerate(self.transformer_encoder[:layer]):
            if i == len(self.transformer_encoder[:layer]) - 1:
                break
            x = blk(x, self.mask[:Sx, :Sx].type_as(x) if self.masked else None) # (Sx, B, E)
        return self.transformer_encoder[layer].attention_map(x, shape_P=shape_P, cls_token=self.cls_token is not None)
...
class Attention(Module):
    ...
    def forward(self, x, attn_mask=None, attention_only=False):
        N, B, C = x.shape
    ...
    def attention_map(self, x, attn_mask=None, discard_cls_token=True, shape_P=None):
        x = self(x, attention_only=True)
        #x = x if not discard_cls_token else x[:-1]
        #x = x[-1]
        #if shape_P is not None:
        #    C, Ph, Pw = shape_P
        #    x = x.reshape(*x.shape[:-1], *shape_P) # reassamble patches
        #    B = x.shape[1]
        #    H = int(np.sqrt(x.shape[0]))
        #    W = H
        #    x = nn.functional.interpolate(x.unsqueeze(0), scale_factor=(1, 96//Pw, 96//Ph), mode="nearest")[0].detach().cpu().numpy()
        #    #new_x = torch.empty(B, C, H*Ph, W*Pw)
        #    #for r in range(H):
        #    #    for c in range(W):
        #    #        idx = c*r + c
        #    #        new_x[:,:,r*Ph:(r+1)*Ph, c*Pw:(c+1)*Pw] = x[idx]
        #    #x = new_x
        return x
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 attention_dropout=0.1, drop_path_rate=0.1, activation= "gelu"):
    ...
    def attention_map(self, src, attn_mask=None, shape_P=None, cls_token=False):
        return self.self_attn.attention_map(self.pre_norm(src), attn_mask, shape_P=shape_P, discard_cls_token=cls_token)
