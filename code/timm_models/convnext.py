...
sanitize = lambda r: (r / r.sum()) #* rel_R_mass
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        if not norm_layer:
            norm_layer = partial(LayerNorm2d, eps=1e-6) if conv_mlp else partial(LayerNorm, eps=1e-6)
    ...
    def relprop(self, R, alpha):
        ...
        R, shortcut = self.add.relprop(R, alpha)
        R = (self.drop_path.relprop(R, alpha))
        if self.gamma is not None:
            R = R# / self.gamma.reshape(1, -1, 1, 1)
        if self.use_conv_mlp:
            R = (self.mlp.relprop(R, alpha))
            ...
            R = (self.norm.relprop(R, alpha))
            R = R.permute(0, 3, 1, 2)
        R = (self.conv_dw.relprop(R, alpha))
        R = sanitize(R + shortcut)
        return R
...
class ConvNeXtStage(nn.Module):
    ...
    def __init__(
        ...
        for i in range(depth):
            ...
            in_chs = out_chs
        self.blocks = Sequential(*stage_blocks)
    ...
    def relprop(self, R, alpha=None):
        R = sanitize(self.blocks.relprop(R, alpha))
        R = sanitize(self.downsample.relprop(R, alpha))
        #R = R / R.sum()
        return R
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer is None:
            ...
            norm_layer_cl = norm_layer if conv_mlp else partial(LayerNorm, eps=1e-6)
        else:
            assert conv_mlp,\
        ...
        if stem_type == 'patch':
            ...
            self.stem = Sequential(
                Conv2d(in_chans, dims[0], kernel_size=stem_kernel_size, stride=stem_stride, bias=conv_bias),
                norm_layer(dims[0])
            )
        else:
            self.stem = Sequential(
        ...
        self.norm_pre = norm_layer(self.num_features) if head_norm_first else Identity()
        self.head = Sequential(OrderedDict([
                ('global_pool', SelectAdaptivePool2d(pool_type=global_pool)),
                ('norm', Identity() if head_norm_first else norm_layer(self.num_features)),
                ('flatten', Flatten(1) if global_pool else Identity()),
                ('drop', Dropout(self.drop_rate)),
                ('fc', Linear(self.num_features, num_classes) if num_classes > 0 else Identity())]))
    ...
    def reset_classifier(self, num_classes=0, global_pool=None):
        if global_pool is not None:
            ...
            self.head.flatten = Flatten(1) if global_pool else Identity()
        self.head.fc = Linear(self.num_features, num_classes) if num_classes > 0 else Identity()
    ...
    def relprop(self, R, alpha=None):
        # head
        R = R if self.pre_logits_used else self.head.fc.relprop(R, alpha, use_epsilon=True)
        R = R/R.sum()
        R = sanitize(self.head.drop.relprop(R, alpha))
        R = sanitize(self.head.flatten.relprop(R, alpha))
        R = sanitize(self.head.norm.relprop(R, alpha))
        R = sanitize(self.head.global_pool.relprop(R, alpha))
        # features
        R = sanitize(self.norm_pre.relprop(R, alpha))
        R = sanitize(self.stages.relprop(R, alpha))
        R = sanitize(self.stem.relprop(R, alpha))
        return R
