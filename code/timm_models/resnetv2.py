...
class PreActBottleneck(nn.Module):
    ...
    def relprop(self, R, alpha=None):
        R, shortcut = self.add.relprop(R, alpha=alpha)
        R = self.drop_path.relprop(R, alpha=alpha)
        R = self.conv3.relprop(R, alpha=alpha)
        R = self.norm3.relprop(R, alpha=alpha)
        R = self.conv2.relprop(R, alpha=alpha)
        R = self.norm2.relprop(R, alpha=alpha)
        R = self.conv1.relprop(R, alpha=alpha)
        R = self.norm1.relprop(R, alpha=alpha)
        if self.downsample is not None:
            shortcut = self.downsample.relprop(shortcut, alpha=alpha)
        return R + shortcut
...
class Bottleneck(nn.Module):
    ...
    def relprop(self, R, alpha):
        R = self.act3.relprop(R, alpha=alpha)
        R = self.drop_path.relprop(R, alpha=alpha)
        R = self.norm3.relprop(R, alpha=alpha)
        R = self.norm2.relprop(R, alpha=alpha)
        R = self.conv2.relprop(R, alpha=alpha)
...
class DownsampleConv(nn.Module):
    ...
    def relprop(self, R, alpha=None):
        ...
        R = self.norm.relprop(R, alpha=alpha)
        return R
...
class ResNetStage(nn.Module):
    ...
    def relprop(self, R, alpha=None):
        return self.blocks.relprop(R, alpha=alpha)
...
def create_resnetv2_stem(
    ...
    if 'fixed' in stem_type:
        ...
        stem['pad'] = ConstantPad2d(1, 0.)
        stem['pool'] = MaxPool2d(kernel_size=3, stride=2, padding=0)
        ...
        stem['pool'] = MaxPool2d(kernel_size=3, stride=2, padding=1)

    return Sequential(stem)
...
class ResNetV2(nn.Module):
    ...
    def __init__(
            ...
            act_layer=ReLU, conv_layer=StdConv2d, norm_layer=partial(GroupNormAct, num_groups=32),
    ...
    def relprop(self, R, alpha=None):
        ...
        R = self.norm.relprop(R, alpha=alpha)
        R = self.stages.relprop(R, alpha=alpha)
        R = self.stem.relprop(R, alpha=alpha)
        return R


def _init_weights(module: nn.Module, name: str = '', zero_init_last=True):
    if isinstance(module, Linear) or ('head.fc' in name and isinstance(module, Conv2d)):
        ...
        if module.bias is not None:
            nn.init.zeros_(module.bias)
    elif isinstance(module, (BatchNorm2d, LayerNorm, GroupNorm)):
...
def _load_weights(model: nn.Module, checkpoint_path: str, prefix: str = 'resnet/'):
    ...
    if isinstance(getattr(model.head, 'fc', None), Conv2d) and \
