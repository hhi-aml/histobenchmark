...
class BasicBlock(nn.Module):
    ...
    def relprop(self, R, alpha=None):
        R = self.act2.relprop(x, alpha)
        R /= R.sum()
        R, shortcut = self.add.relprop(R, alpha) 
        if self.downsample is not None:
            shortcut = self.downsample.relprop(shortcut, alpha)
        if self.drop_path is not None:
            R = self.drop_path.relprop(R, alpha)
        if self.se is not None:
            R = self.se.relprop(R, alpha)
        R = self.bn2.relprop(R, alpha)
        R = self.conv2.relprop(R, alpha)
        R = self.aa.relprop(R, alpha)
        R = self.act1.relprop(R, alpha)
        R = self.drop_block.relprop(R, alpha)
...
class Bottleneck(nn.Module):
    ...
    def relprop(self, R, alpha):
        ...
        if self.se is not None:
            R = self.se.relprop(R, alpha)
        R = self.bn3.relprop(R, alpha)
        R = self.conv3.relprop(R, alpha)
        R = self.aa.relprop(R, alpha)
        R = self.act2.relprop(R, alpha)
        R = self.drop_block.relprop(R, alpha)
        R = self.bn2.relprop(R, alpha)
        R = self.conv2.relprop(R, alpha)
        R = self.act1.relprop(R, alpha)
...
class ResNet(nn.Module):
    ...
    def relprop(self, R, alpha):
        ...
        if torch.isnan(R).sum() > 0 or torch.isinf(R).sum() > 0:
            raise
        R /= R.sum()
