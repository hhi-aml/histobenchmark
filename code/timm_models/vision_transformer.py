...
class Attention(nn.Module):
    ...
    def get_class_token_attn_weights(self, x, P=16, S_in=224):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # q, k, v of shape BxHxNx(C//H)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        # attn of shape BxHxNxN
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block(nn.Module):
    ...
    def get_attn_maps(self, x, P=16, S_in=224, interpolation='bicubic'):
        attn_weights = self.attn.get_class_token_attn_weights(self.norm1(x), P=P, S_in=S_in)
        attn_weights = attn_weights.reshape(attn_weights.shape[0], attn_weights.shape[1], S_in//P, S_in//P)
        B, H, _, _ = attn_weights.shape
        attn_map = torch.zeros(B, H, S_in, S_in, dtype=torch.float32)
        str_to_intp = {'nearest': 0, 'bilinear': 2, 'bicubic': 3}
        for b in range(B):
            for h in range(H):
                attn_map[b,h] = F_trafo.resize(attn_weights[b,h][None,:,:], size=[S_in, S_in], 
                                               interpolation=str_to_intp[interpolation])[0]
        return attn_map
